...
define(["AppChart"], function(AppChart) {
   ...
   *   implementation. An object with any of the following properties: <ul>
   *   <li>dataTransformer - a function supplying a callback to transform
   *     chart data from a raw representation into a representation the
   ...
   * @static
   * @param options a JSON object with the following properties: <ul>
   *   <li>dataTransformer - a function supplying a callback to transform
   *     chart data from a raw representation into a representation the
   *     AppChart class can deal with</li>
   *   <li>ctrlName - the name of the controller object in the AngularJS scope
   ...
   *   </ul>
   */
  AppChartController.getFactory = function(options) {
    return function($scope, $element) {
      return new AppChartController($scope, $element, options);
    };
  };
   ...
   */
  AppChartController.prototype.link = function() {
  ...
  AppChartController.prototype.dataUpdated = function() {
    if (this.chart)
      this.chart.bind(this.data, this.options.dataTransformer);
  };
